...
const IndustriesSection = ({ homepage, data, isFirst, isLast, sectionIndex, pageType }) => {
  ...
  const transformStrapiData = (strapiData) => {
    return strapiData.map((item, index) => {
      const defaultIndustry = defaultIndustries[index] || defaultIndustries[0];
      
      return {
        title: item.title || defaultIndustry.title,
        description: item.description || defaultIndustry.description,
        ctaLabel: item.ctaLabel || defaultIndustry.ctaLabel,
        background: item.background?.url ? `${import.meta.env.VITE_STRAPI_URL || 'http://localhost:1337'}${item.background.url}` : defaultIndustry.background,
        type: item.type || defaultIndustry.type,
        metrics: item.metrics || defaultIndustry.metrics
      };
    });
  };

  // Fetch from Strapi with fallback using enhanced API
  const { data: strapiIndustries, loading: strapiLoading, strapiAvailable } = useStrapiData(
    '/api/industries',
    defaultIndustries,
    {
      useStrapiAPI: true,
  ...
  const industries = useMemo(() => {
    return strapiLoading ? defaultIndustries : strapiIndustries;
  }, [strapiLoading, strapiIndustries]);
            ...
            <div className="left_top_content pe-2 pe-md-3">
              <div className="sec_heading_wrap">
                ...
                <div className="heading_text mt-3">
                  <p>We serve data-critical industries where context and compliance matter.</p>
                </div>
              </div>
            </div>
